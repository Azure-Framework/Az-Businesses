...
local function makeVector4(tbl)
    ...
    return vector4(tbl.x or tbl[1] or 0.0, tbl.y or tbl[2] or 0.0, tbl.z or tbl[3] or 0.0, tbl.w or tbl[4] or 0.0)
end
...
local function createPlayerBusiness(src, customLabel)
    ...
    if not charId then
        ...
        TriggerClientEvent(
            ...
            src,
            {
                args = {"^1[Business]^7 You must have an active character to create a business."}
            }
        )
        return
    end
    ...
    if not ped or ped == 0 then
        ...
        TriggerClientEvent(
            ...
            src,
            {
                args = {"^1[Business]^7 Could not resolve your position."}
            }
        )
        return
    end
    ...
    if cost > 0 then
        ...
        if not ok then
            TriggerClientEvent(
                ...
                src,
                {
                    args = {"^1[Business]^7 Failed to process payment."}
                }
            )
            return
        end
    ...
    TriggerClientEvent(
        ...
        {
            args = {
                ("^2[Business]^7 You created a new business: ^3%s"):format(label)
            }
        }
    )
end
...
RegisterNetEvent(
    ...
    function(slotId)
        ...
        if biz.ownerCharId then
            TriggerClientEvent("chat:addMessage", src, {args = {"^1[Business]^7 This business is already owned."}})
            return
        end

        local price = tonumber(biz.buyPrice or 0) or 0
        if price <= 0 then
            TriggerClientEvent("chat:addMessage", src, {args = {"^1[Business]^7 This business is not for sale."}})
            return
        end

        local charId = getPlayerCharId(src)
        if not charId then
            TriggerClientEvent("chat:addMessage", src, {args = {"^1[Business]^7 You must have an active character."}})
            return
        end
        ...
        if not ok then
            TriggerClientEvent("chat:addMessage", src, {args = {"^1[Business]^7 Failed to process payment."}})
            return
        end
        ...
        TriggerClientEvent(
            ...
            {
                args = {
                    ("^2[Business]^7 You purchased ^3%s^7 for ^2$%s^7."):format(biz.label, tostring(price))
                }
            }
        )
...
RegisterNetEvent(
    ...
    function(slotId, itemId)
        ...
        if not item then
            TriggerClientEvent("chat:addMessage", src, {args = {"^1[Business]^7 Item not found."}})
            return
        end

        local price = tonumber(item.price or 0) or 0
        if price <= 0 then
            TriggerClientEvent("chat:addMessage", src, {args = {"^1[Business]^7 This item is not for sale."}})
            return
        end
        ...
        TriggerClientEvent(
            ...
            {
                args = {
                    ("^2[Business]^7 Purchased ^3%s^7 for ^2$%s^7. (RP item, manual)"):format(
...
RegisterNetEvent(
    "az-biz:addInventoryItem",
    function(slotId, itemId, label, price)
        local src = source
        local biz = Businesses[slotId]
        if not biz then
            return
        ...
        if role ~= "owner" and role ~= "manager" then
            TriggerClientEvent(
                "chat:addMessage",
                src,
                {args = {"^1[Business]^7 You are not allowed to manage inventory."}}
            )
            return
        end
        ...
        if itemId == "" or label == "" or price < 0 then
            TriggerClientEvent("chat:addMessage", src, {args = {"^1[Business]^7 Invalid item data."}})
            return
        end
        ...
        TriggerClientEvent(
            ...
            {
                args = {
                    ("^2[Business]^7 Stock updated: ^3%s^7 ($%s)."):format(label, tostring(price))
                }
            }
        )
    end
...
RegisterNetEvent(
    ...
    function(slotId, itemId)
        ...
        if removed then
            ...
            TriggerClientEvent("chat:addMessage", src, {args = {"^2[Business]^7 Item removed from stock."}})
        else
            TriggerClientEvent("chat:addMessage", src, {args = {"^1[Business]^7 Item not found in stock."}})
        end
    end
)

RegisterNetEvent(
    "az-biz:withdrawFunds",
    function(slotId, amount)
        local src = source
        local biz = Businesses[slotId]
        if not biz then
            return
        ...
        local role = getRoleForChar(biz, charId)
        if role ~= "owner" and role ~= "manager" then
            TriggerClientEvent("chat:addMessage", src, {args = {"^1[Business]^7 You are not allowed to withdraw."}})
            return
        end
        ...
        if (biz.balance or 0) < amount then
            TriggerClientEvent("chat:addMessage", src, {args = {"^1[Business]^7 Not enough business funds."}})
            return
        end
        ...
        TriggerClientEvent(
            ...
            {
                args = {
                    ("^2[Business]^7 You withdrew ^2$%s^7 from ^3%s^7."):format(tostring(amount), biz.label)
                }
            }
        )
    end
    ...
    function(slotId, amount)
        ...
        if role ~= "owner" and role ~= "manager" then
            TriggerClientEvent(
                "chat:addMessage",
                src,
                {args = {"^1[Business]^7 You are not allowed to deposit into this business."}}
            )
            return
        end
        ...
        if amount <= 0 then
            TriggerClientEvent(
                "chat:addMessage",
                src,
                {args = {"^1[Business]^7 Deposit amount must be greater than 0."}}
            )
            return
        end
        ...
        if not ok then
            TriggerClientEvent("chat:addMessage", src, {args = {"^1[Business]^7 Failed to process deposit."}})
            return
        end
        ...
        TriggerClientEvent(
            ...
            {
                args = {
                    ("^2[Business]^7 You deposited ^2$%s^7 into ^3%s^7."):format(tostring(amount), biz.label)
                }
            }
        )
    end
    ...
    function(slotId, serverId, role)
        ...
        if myRole ~= "owner" and myRole ~= "manager" then
            TriggerClientEvent(
                "chat:addMessage",
                src,
                {args = {"^1[Business]^7 You are not allowed to hire employees."}}
            )
            return
        end
        ...
        if targetSrc <= 0 or not GetPlayerPed(targetSrc) then
            TriggerClientEvent("chat:addMessage", src, {args = {"^1[Business]^7 Invalid or offline player id."}})
            return
        end

        local targetCharId = getPlayerCharId(targetSrc)
        if not targetCharId then
            TriggerClientEvent("chat:addMessage", src, {args = {"^1[Business]^7 Target has no active character."}})
            return
        end
        ...
        TriggerClientEvent(
            ...
            {
                args = {
                    ("^2[Business]^7 Hired ^3%s^7 as ^2%s^7."):format(fullName, role:upper())
                }
            }
        )
    end
...
RegisterNetEvent(
    "az-biz:removeEmployee",
    function(slotId, remCharId)
        local src = source
        local biz = Businesses[slotId]
        if not biz then
            return
        ...
        if myRole ~= "owner" and myRole ~= "manager" then
            TriggerClientEvent(
                "chat:addMessage",
                src,
                {args = {"^1[Business]^7 You are not allowed to manage employees."}}
            )
            return
        end
        ...
        if removed then
            ...
            TriggerClientEvent(
                ...
                {
                    args = {
                        ("^2[Business]^7 Removed employee ^3%s^7."):format(removedName or remCharId)
                    }
                }
            )
        else
            TriggerClientEvent("chat:addMessage", src, {args = {"^1[Business]^7 Employee not found."}})
        end
    end
)
    ...
    function(slotId, message)
        ...
        if role ~= "owner" and role ~= "manager" then
            TriggerClientEvent(
                "chat:addMessage",
                src,
                {args = {"^1[Business]^7 You are not allowed to promote this business."}}
            )
            return
        end
        ...
        if message == "" then
            TriggerClientEvent(
                "chat:addMessage",
                src,
                {args = {"^1[Business]^7 Advertisement message cannot be empty."}}
            )
            return
        end
        ...
        if (biz.balance or 0) < fee then
            TriggerClientEvent(
                ...
                {
                    args = {
                        ("^1[Business]^7 Not enough business funds to advertise (need $%s)."):format(tostring(fee))
                    }
                }
            )
            return
        ...
        TriggerClientEvent(
            ...
            {
                args = {
                    ("^2[Business]^7 Advert sent. Cost: ^1$%s^7 from business funds."):format(tostring(fee))
                }
            }
        )
    end
...
RegisterNetEvent(
    "az-biz:updateInfo",
    function(slotId, newLabel, newType, newStatus)
        local src = source
        local biz = Businesses[slotId]
        if not biz then
            return
        ...
        if role ~= "owner" and role ~= "manager" then
            TriggerClientEvent(
                "chat:addMessage",
                src,
                {args = {"^1[Business]^7 You are not allowed to edit this business."}}
            )
            return
        end
        ...
        TriggerClientEvent("chat:addMessage", src, {args = {"^2[Business]^7 Business info updated."}})
    end
)
