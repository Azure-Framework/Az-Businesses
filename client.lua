...
local function makeVector4(coords)
    if type(coords) == "vector4" or type(coords) == "userdata" then
        return coords
    end
                ...
                if dist < (Config.DrawDistance or 30.0) then
                    ...
                    if dist < (Config.InteractDistance or 2.0) then
                        ...
                        if not biz.ownerCharId and (biz.buyPrice or 0) > 0 then
                            drawHelpText(
                                ("Press ~INPUT_CONTEXT~ to purchase ~y~%s~s~ for ~g~$%s~s~"):format(
                                    label,
                                    tostring(biz.buyPrice or 0)
                                )
                            )
                        else
